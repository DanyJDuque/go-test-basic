...
import (
	...
	"github.com/DanyJDuque/go-test-basic/internal/logic"
	"github.com/stretchr/testify/assert"
)

func TestMain(m *testing.M) {
	fmt.Println("Starting tests")
	exitCode := m.Run() // recibimos el valor de ejecuatar los test
	fmt.Println("Tests finished")
	os.Exit(exitCode) // Finalizo el programa
...
func TestCheckEmail(t *testing.T) {
	...
	invalidEmail := "testtest.com"
	got = logic.CheckEmail(invalidEmail)
	expected = false
	assert.Equal(t, expected, got)

}
func TestCheckEmail2(t *testing.T) {
	email := "test@test.com"
	got := logic.CheckEmail(email)
	assert.True(t, got)
